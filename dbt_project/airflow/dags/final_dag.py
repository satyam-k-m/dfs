...
DIM_PIPELINE_TABLE = "dim_pipeline"
DIM_TASK_TABLE = "dim_task"
FCT_PIPELINE_TABLE = "fct_pipeline"
FCT_TASKS_TABLE = "fct_task"
AUDIT_DB = "DFS_POC"
AUDIT_SCHEMA = "AIRFLOW_AUDIT"
...
SQL_REFRESH_STATEMENT = "ALTER EXTERNAL TABLE %(AUDIT_DB)s.%(AUDIT_SCHEMA)s.%(table_name)s REFRESH"
    ...
    catchup=False,
        ...
        with TaskGroup("setup_audit_config") as tg:
            ...
            read_config_table = SnowflakeOperator(
                ...
                params = {"table_name": DIM_TASK_TABLE, 'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
            ...
            create_fact_pipeline = SnowflakeOperator(
                ...
                params = {"table_name":FCT_PIPELINE_TABLE,  'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
            )
            create_fact_task = SnowflakeOperator(
                task_id = "create_fct_task",
                sql = sql_stmts.create_fct_task,
                params = {"table_name":FCT_TASKS_TABLE,  'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
            )

            insert_pipeline_fact = SnowflakeOperator(
                task_id = "insert_fct_pipeline",
                sql = sql_stmts.insert_pipeline_status,
                params = {"fact_table_name":FCT_PIPELINE_TABLE, 
                        "dim_table_name":DIM_PIPELINE_TABLE, 
                        "pipeline_id": dag.dag_id, "run_ts":start_date_var,  'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
            )
            insert_task_fact = SnowflakeOperator(
                task_id = "insert_fct_task",
                sql = sql_stmts.insert_task_status,
                params = {"fact_table_name":FCT_TASKS_TABLE, 
                        "dim_table_name":DIM_TASK_TABLE, 
                        "pipeline_id": dag.dag_id,"run_ts":start_date_var,  'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
            )
        ...
        update_fact_pipeline_pre = SnowflakeOperator(
            ...
            params = {"table_name":FCT_PIPELINE_TABLE, "status_cd":"running", 
                    "pipeline_id":dag.dag_id, "run_ts":start_date_var,  'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
        )
        update_fact_pipeline_post = SnowflakeOperator(
            ...
            params = {"table_name":FCT_PIPELINE_TABLE, "status_cd":"success", 
                    "pipeline_id":dag.dag_id, "run_ts":start_date_var,  'db':AUDIT_DB, "schema":AUDIT_SCHEMA}
        )
